...
import baxter_interface	#Baxter接口
# 在baxter_external_devices导入到使用它的getch功能，捕获键盘上的按键。 也就是说，如果需要除了Baxter之外的设备信号，需要导入这个库
...
#初始化结点，检查版本
    ...
    def clean_shutdown():
        print("\nExiting example...")
        if not init_state:# 如果机器人一开始不是enabled，结束时恢复disable
